...
class MOTSDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(self.distill_img_ids) == 0:
            distill_img = np.zeros([1, self.crop_d, self.crop_h, self.crop_w])
            distill_tid = -1
        else:
            distill_img_path = random.choice(self.distill_img_ids)
            fname = distill_img_path.split("/")[-1]
            if "noise" in fname:
                distill_tid = random.sample([0, 1, 3, 6], 1)[0]
            else:
                distill_tid = int(fname[4])
            distill_tid = modify_task_id(distill_tid)
            distill_img = nib.load(distill_img_path).get_data()
            distill_img = distill_img[np.newaxis, :]

        # print(distill_img.shape, image.shape)

        if self.is_mirror:
            if np.random.rand(1) <= 0.5:  # flip W
                distill_img = distill_img[:, :, :, ::-1]
                image = image[:, :, :, ::-1]
                label = label[:, :, :, ::-1]
        ...
        distill_img = distill_img.astype(np.float32)
...
def get_train_transform():
    ...
    # tr_transforms.append(GaussianNoiseTransform(p_per_sample=0.1, data_key="distill_img"))
    # tr_transforms.append(
    #     GaussianBlurTransform(blur_sigma=(0.5, 1.), different_sigma_per_channel=True, p_per_channel=0.5,
    #                           p_per_sample=0.2, data_key="distill_img"))
    # tr_transforms.append(BrightnessMultiplicativeTransform((0.75, 1.25), p_per_sample=0.15, data_key="distill_img"))
    # tr_transforms.append(BrightnessTransform(0.0, 0.1, True, p_per_sample=0.15, p_per_channel=0.5, data_key="distill_img"))
    # tr_transforms.append(ContrastAugmentationTransform(p_per_sample=0.15, data_key="distill_img"))
    # tr_transforms.append(
    #     SimulateLowResolutionTransform(zoom_range=(0.5, 1), per_channel=True, p_per_channel=0.5, order_downsample=0,
    #                                    order_upsample=3, p_per_sample=0.25,
    #                                    ignore_axes=None, data_key="distill_img"))
    # tr_transforms.append(GammaTransform(gamma_range=(0.7, 1.5), invert_image=False, per_channel=True, retain_stats=True,
    #                                     p_per_sample=0.15, data_key="distill_img"))
...
def my_collate(batch):
    image, label, name, task_id, distill_img, distill_tid = zip(*batch)
    # for e in distill_img:
    #     print(e.shape)
    distill_img = np.stack(distill_img, 0)
    image = np.stack(image, 0)
    label = np.stack(label, 0)
    name = np.stack(name, 0)
    task_id = np.stack(task_id, 0)
    distill_tid = np.stack(distill_tid, 0)
    data_dict = {'image': image, \
                 'label': label,
                 'name': name,
                 'task_id': task_id,
                 'distill_img': distill_img,
                 'distill_tid': distill_tid}
