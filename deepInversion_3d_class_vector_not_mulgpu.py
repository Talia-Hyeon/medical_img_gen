...
def gen_img_vector(args, device, task_id=1):
    ...
    # make directory
    root_p = f"./sample/time_test/{train_type}/{task_id}"
    ...
    while cnt < n_imgs:
        ...
        iter_end = time()
        report = dict()
        report['time'] = iter_end - iter_start
        ...
        if organ_pixels / pixels >= organ_percentage and cnt < n_imgs:
            ...
            cnt += 1
        print('ratio of foreground: {:.5f}%'.format((organ_pixels / pixels) * 100))
        time_cnt += 1
        writer.add_scalars('Time', report, time_cnt)


def gen_img_args():
    parser = argparse.ArgumentParser(description="image generation")
    parser.add_argument("--type", type=str, default='hrhf')
    parser.add_argument("--task_id", type=int, default=4)
    parser.add_argument("--gpu", type=str, default='0')
    parser.add_argument("--gen_epochs", type=int, default=2000)
    parser.add_argument("--cnt", type=int, default=0)
    parser.add_argument("--num_imgs", type=int, default=400)
