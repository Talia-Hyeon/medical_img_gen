...
class PseudoVis(data.Dataset):
    ...
    def save_png(self, npy_img_path):
        ...
        if npy_label_path.find('final') != -1:
            npy_target_path = npy_img_path.replace('img', 'label')
            target = np.load(npy_target_path)
            target = target.astype(np.float32)
            self.visualization(image, label, img_iter, img_dir, target)

        else:
            self.visualization(image, label, img_iter, img_dir)

    def visualization(self, npy_img, npy_label, img_iter, img_dir, npy_target=None):
        # remove batch
